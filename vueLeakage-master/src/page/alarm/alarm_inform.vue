...
<style>
    ...
    .loncom_system_inform_card li {
      width: calc(33.333% - 20px); /* 设置每个li标签的宽度为容器宽度的三分之一减去间隔 */
      margin-right: 10px; /* 右边距 */
      margin-bottom: 10px; /* 底部距，如果需要 */
      box-sizing: border-box; /* 包括padding和border在内的宽度计算 */
    }

    /* 最后一个li标签右边距为0，如果需要 */
    .loncom_system_inform_card li:last-child {
      margin-right: 0;
    }
    ...
    @media (min-width: 1200px) {
      .loncom_system_inform_card li {
        width: calc(25% - 20px); /* 在大屏幕上，每个li标签的宽度为容器宽度的四分之一减去间隔 */
      }
    }
...
export default {
    ...
    data() {
       return {
            table_columns:[
              ...
              { prop: 'system_dx', label: '邮箱号',minWidth:10},
              { prop: 'system_sg', label: '邮箱授权码',minWidth:10},
              { prop: 'system_dh', label: '服务器域名',minWidth:10},
              { prop: 'system_yj', label: '邮件',minWidth:10,slotName:'preview-yj'},
              { prop: 'system_time', label: '发送时间',minWidth:15},
              { prop: 'system_user', label: '告警接收人',minWidth:15},
           ],
           table_data:[
               {system_yj:true,system_time:'',system_user:""},
               {system_dx:true,system_sg:false,system_dh:false,system_yj:false,system_time:'',system_user:""},
               {system_dx:false,system_sg:true,system_dh:false,system_yj:false,system_time:'',system_user:""},
               {system_dx:false,system_sg:false,system_dh:true,system_yj:false,system_time:'',system_user:""},
           ],
    ...
    methods:{
      fetchData() {
        axios.get('http://localhost:3000/email/getall', {
          headers: {
            'Authorization': `Bearer ${this.token}`
          }})
          .then(response => {
            let data = Array.isArray(response.data) ? response.data : [response.data];
            // 假设后端返回的数据格式为数组，每个元素包含上述属性
            this.table_data = data.map(item => ({
              system_dx: item.sendEmail,
              system_sg: item.sendCode,
              system_dh: 'smtp.qq.com',
              system_yj: true, // 邮件属性写死为true
              system_time: item.sendTime,
              system_user: item.receiveEmail,
            }));
          })
          .catch(error => {
